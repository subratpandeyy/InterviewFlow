...
export async function getInterviewerTokens(interviewerId: string): Promise<GoogleTokenData | null> {
  ...
  const refreshTokenDecrypted = decryptToken(tokenRow.refresh_token);
  const tokenExpiresAt = new Date(tokenRow.token_expires_at).getTime();

  if (Date.now() >= tokenExpiresAt) {
    const refreshed = await refreshAccessToken(refreshTokenDecrypted);
...
export async function createCalendarEvent(
  ...
  try {
    ...
    throw new ExternalApiError('Google Calendar', 'Failed to create event', {
...
export async function updateCalendarEvent(
  ...
  try {
    ...
    throw new ExternalApiError('Google Calendar', 'Failed to update event', {
      error: err instanceof Error ? err.message : 'Unknown',
    });
  }
}
...
export async function deleteCalendarEvent(
  ...
  try {
    ...
    throw new ExternalApiError('Google Calendar', 'Failed to delete event', {
      error: err instanceof Error ? err.message : 'Unknown',
    });
  }
}
...
export async function listUpcomingEvents(
  ...
  try {
    ...
    throw new ExternalApiError('Google Calendar', 'Failed to list events', {
      error: err instanceof Error ? err.message : 'Unknown',
    });
  }
}
...
export async function updateCalendarSyncStatus(
  ...
  try {
    await admin.from('google_calendar_tokens').update(update).eq('profile_id', profileId);
  } catch {
