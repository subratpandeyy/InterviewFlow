...
import { Upload, Trash2, FileText, FileImage, FileArchive, File, Download } from 'lucide-react';
...
export function PortalDocuments({ documents, sessionToken }: { documents: CandidateDocument[]; sessionToken: string }) {
  ...
  const handleUpload = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await actions.uploadPortalDocument(formData);
  ...
  const handleDelete = (doc: CandidateDocument) => {
    startTransition(async () => {
      ...
      const result = await actions.deletePortalDocument(formData);
      if ('error' in result && result.error) {
        toast.error(result.error);
      } else {
              ...
              {documents.map((doc) => (
                <div key={doc.id} className="flex items-center gap-3 p-3 rounded-lg border border-border bg-muted/20">
                  ...
                  <div className="flex items-center gap-1">
                    <a href={doc.file_url} target="_blank" rel="noopener noreferrer" download={doc.file_name || undefined}
      ...
      <Dialog open={showUpload} onOpenChange={(open) => { if (!open) setShowUpload(false); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Upload a file to your profile</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleUpload} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="document_type">Document Type</Label>
              <select id="document_type" name="document_type" required className="flex h-8 w-full rounded-lg border border-input bg-transparent px-2.5 py-1 text-sm">
            ...
            <div className="space-y-2">
              ...
              <input ref={fileInputRef} id="file" name="file" type="file" required
            ...
            <DialogFooter>
              ...
              <LoadingButton type="submit" loading={uploading} loadingText="Uploading...">Upload</LoadingButton>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
